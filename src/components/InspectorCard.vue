...
<template>
  ...
  >
    ...
    <dl v-if="strut" class="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1.5 px-3 py-3 text-xs">
      ...
      <dd class="text-right font-mono font-semibold text-sm">
        {{ formatLength(strut.row.roundedCutLength, state.units) }}
      </dd>
      ...
      <dd class="text-right font-mono">
        {{
          strut.row.roundingError < 1e-9 ? '0' : formatLength(strut.row.roundingError, state.units)
        }}
      </dd>
      ...
      <dd class="text-right font-mono">
        {{
          Number.isNaN(strut.type.dihedralMinDeg)
            ? '—'
            : `${strut.type.dihedralMinDeg.toFixed(1)}–${strut.type.dihedralMaxDeg.toFixed(1)}°`
        }}
      </dd>
