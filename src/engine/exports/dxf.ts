...
export function fabricationDxf(model: DomeModel, cutList: CutList, radius: number): string {
  ...
  const line = (layer: string, x1: number, y1: number, x2: number, y2: number) =>
    push(0, 'LINE', 8, layer, 10, x1.toFixed(4), 20, y1.toFixed(4), 30, 0, 11, x2.toFixed(4), 21, y2.toFixed(4), 31, 0)
  ...
  cutList.rows.forEach((r, i) => {
    ...
    text('struts', 0, y + gap * 0.2, gap * 0.18,
  ...
  return [
    ...
    0, 'LAYER', 2, 'struts', 70, 0, 62, 2, 6, 'CONTINUOUS',
    0, 'LAYER', 2, 'dome_plan', 70, 0, 62, 5, 6, 'CONTINUOUS',
    0, 'ENDTAB', 0, 'ENDSEC',
